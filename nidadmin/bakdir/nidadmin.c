...
void privkey_reply(SSL *ssl)
{
	...
	mysql_login_my(&mysql,"211.68.122.23","root","YUIOPPOIUY","cngi");
	mysql_query_prikey(&mysql,prikey,&prikeylen);
	printf("长度:%d, 密钥:%s\n",prikeylen,prikey);
	mysql_close_my(&mysql);
...
void update_tuples(SSL *ssl,struct packet *recvbuf,uint32_t total_len)
{
	...
	mysql_login_my(&mysql,"211.68.122.23","root","YUIOPPOIUY","cngi");
	mysql_insert_info_to_radpostauth(&mysql,username,ipv6_addr,mac_addr_str,time);
	mysql_close_my(&mysql);
